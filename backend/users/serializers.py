...
class FollowSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        following_id = self.context['view'].kwargs.get('id')  # или 'user_id' в зависимости
        # от URL Djoser

        if not User.objects.filter(id=following_id).exists():
            # Это должно обрабатываться на уровне ViewSet (get_object_or_404)
            raise serializers.ValidationError(
                "Пользователь, на которого вы пытаетесь подписаться, не найден."
            )
...
class SetAvatarSerializer(serializers.ModelSerializer):
    ...
    avatar = serializers.ImageField(
        required=True
    )  # Используем ImageField для валидации и сохранения
