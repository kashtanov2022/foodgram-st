...
class RecipeFilter(filters.FilterSet):
    ...
    class Meta:
        ...
        fields = [
            ...
            'is_favorited',
            'is_in_shopping_cart',
        ]  # Add author if you filter by it
    ...
    def filter_is_favorited(self, queryset, name, value):
        ...
        if user.is_authenticated and value:  # If value is True
            ...
            # If you were using a ManyToManyField directly on Recipe model
            # for favorites:
            # return queryset.filter(favorited_by_users=user).distinct()
            # Adjust field name
        elif user.is_authenticated and not value:  # If value is False
            return queryset.exclude(favorited_by__user=user).distinct()
        return queryset  # Or handle unauthenticated users differently
