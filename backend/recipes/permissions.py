...
class IsAuthorOrReadOnly(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        if request.method in permissions.SAFE_METHODS:
            return True
        # Для остальных методов (POST, PUT, DELETE) пользователь должен быть аутентифицирован
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if request.method in permissions.SAFE_METHODS:
            return True
        # Для методов изменения (PUT, DELETE) проверяем, является ли пользователь автором объекта
